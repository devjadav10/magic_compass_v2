...
class CalibrationView extends StatefulWidget {
  ...
  CalibrationView(
      {required this.calibrationService, required this.navigationService});
...
class _CalibrationViewState extends State<CalibrationView> {
  ...
  void _calibrate(String qrCodeData) {
    ...
    Node node = Node(name: name, x: x, y: y);
    widget.calibrationService.calibrate(node);
    widget.navigationService.setCurrentLocation(node);
    showDialog(
      context: context,
