...
class NavigationView extends StatefulWidget {
  ...
  NavigationView(
      {required this.navigationService, required this.calibrationService});
...
class _NavigationViewState extends State<NavigationView> {
  ...
  double _calculateDirection() {
    Node? currentLocation = widget.navigationService.getCurrentLocation();
    Node? destination = widget.navigationService.getDestination();

    if (currentLocation != null && destination != null) {
      double dx = destination.x - currentLocation.x;
      double dy = destination.y - currentLocation.y;
      double angle = atan2(dy, dx) * (180 / -pi) + 35;
      ...
      body: Center(
        child: Column(
          ...
          children: <Widget>[
            ...
            Transform.rotate(
              angle:
                  ((_direction - _calculateDirection()) * (3.14159 / 180) * -1),
              child: Image.asset('assets/compass.png', width: 200, height: 200),
            ),
            ElevatedButton(
              child:
                ...
                Navigator.push(
                  ...
                  MaterialPageRoute(
                      builder: (context) => CalibrationView(
                          calibrationService: widget.calibrationService,
